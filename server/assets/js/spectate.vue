        ...
        <div :class="{ nine: !fullScreen, twelve: fullScreen }" class="columns">
          <div class="flex">
            <label style="display: inline-block">
              Auto-spectate
              <input style="margin: 0" type="checkbox" v-model="autoSpectate"
            /></label>
            <label style="display: inline-block">
              Full screen
              <input style="margin: 0" type="checkbox" v-model="fullScreen"
            /></label>
            <label style="display: inline-block">
              Performance mode
              <input
            ...
            <progress
              ...
              max="100"
            ></progress>
            <span title="Nombre de débris sur la carte"
              >🔺 {{ debrisCount }} 🔻</span
            >
          </div>
        ...
        <div class="three columns" v-show="!fullScreen">
          <div v-if="focusedPlayer">
            <h3 style="margin-bottom: 0; text-align: center">
              {{ focusedPlayer.name.text }}
            </h3>
            <div
              style="width: 100%; height: 15px"
              :style="{ background: `${focusedPlayer.color}` }"
            ></div>
            <table>
              <tr>
                <td>❤️ HP</td>
                <td>
                  <strong>{{ Math.round(focusedPlayer.current_hp) }}</strong
                  >/{{ focusedPlayer.max_hp }} (LVL
                  {{ focusedPlayer.upgrade_levels.max_hp }})
                </td>
              </tr>
              <tr>
                <td>💣 Projectile damage</td>
                <td>
                  <strong>{{ focusedPlayer.projectile_damage }}</strong> (LVL
                  {{ focusedPlayer.upgrade_levels.projectile_damage }})
                </td>
              </tr>
              <tr>
                <td title="Cooldown between each projectile launch">
                  🕒 Fire rate <span style="font-size: 70%">(?)</span>
                </td>
                <td>
                  <strong>{{ focusedPlayer.fire_rate }}</strong> (LVL
                  {{ focusedPlayer.upgrade_levels.fire_rate }})
                </td>
              </tr>
              <tr>
                <td>🐇 Speed</td>
                <td>
                  <strong>{{ focusedPlayer.speed }}</strong> (LVL
                  {{ focusedPlayer.upgrade_levels.speed }})
                </td>
              </tr>
              <tr>
                <td title="Damage another entity takes upon colliding with it.">
                  👊 Body damage
                </td>
                <td>
                  <strong>{{ focusedPlayer.body_damage }}</strong> (LVL
                  {{ focusedPlayer.upgrade_levels.body_damage }})
                </td>
              </tr>
              <tr>
                <td title="HP regenerated every tick.">💞 HP regen</td>
                <td>
                  <strong>{{ focusedPlayer.hp_regen }}</strong> (LVL
                  {{ focusedPlayer.upgrade_levels.hp_regen }})
                </td>
              </tr>
              <tr>
                <td title="Projectile time to live.">
                  🚀 Projectile time to live
                </td>
                <td>
                  <strong>{{ focusedPlayer.projectile_time_to_live }}</strong>
                  (LVL
                  {{ focusedPlayer.upgrade_levels.projectile_time_to_live }})
                </td>
              </tr>
              <tr>
                <td>🆙 Upgrade tokens available</td>
                <td>
                  <strong>{{ focusedPlayer.upgrade_tokens }}</strong>
                </td>
              </tr>
            </table>
          </div>
          <div>
            ...
            <div
              v-for="(tank, index) in orderedTanks"
              class="flex scoreboard"
              @click="focusPlayer(tank)"
            >
              <strong class="rank">{{ index + 1 }}</strong>
              <input
                class="player-name"
                type="button"
                :value="`${tank.name.text}`"
...
export default {
  ...
  computed: {
     ...
     */
    orderedTanks() {
      return Object.values(this.elements.tanks).sort((a, b) =>
        a.score < b.score ? 1 : -1
      );
    },
  },
  ...
  mounted() {
    ...
    this.mainCanvas.on("mouse:move", (opt) => {
      if (this.mainCanvas.isDragging && !this.lockCamera) {
        const e = opt.e;
        this.mainCanvas.relativePan(
          new fabric.Point(
            e.clientX - this.mainCanvas.lastPosX,
            e.clientY - this.mainCanvas.lastPosY
          )
        );
  ...
  methods: {
    ...
    doFrame() {
      ...
      if (
        this.autoSpectate &&
        now - this.lastUpdateTimestamp > CANVAS_UPDATE_RATE
      ) {
      ...
      updatedProjectiles.forEach((projectile) => {
        ...
        if (!this.elements.projectiles[projectile.id]) {
          const newProjectile = new Projectile(
            this.elements.tanks[projectile.owner_id],
            projectile
          );
          this.elements.projectiles[projectile.id] = newProjectile;
          const tank_index = this.mainCanvas
            .getObjects()
            .indexOf(this.elements.tanks[projectile.owner_id].toCanvas);
    ...
    animateCanvas(updatedGameState) {
      this.progress =
        updatedGameState.clock.current_tick / updatedGameState.clock.max_tick;
    ...
    hideIfUnzoomed() {
      ...
      Object.values(this.elements.tanks).forEach((tank) =>
        tank.setHUDVisible(isVisible)
      );
    },
    doZoom(event = null) {
    ...
    centerPan() {
      if (this.lockCamera && this.focusedPlayer) {
        this.mainCanvas.absolutePan(
          new fabric.Point(
            this.focusedPlayer.left() * this.zoom -
              this.mainCanvas.getWidth() / 2,
            this.focusedPlayer.top() * this.zoom -
              this.mainCanvas.getHeight() / 2
          )
        );
      }
    },
    ...
    renderMinimap() {
      ...
      this.minimap.viewPort.left =
        canvasWidth / 2 -
        this.mainCanvas.viewportTransform[4] * (1 / this.zoom);
      this.minimap.viewPort.top =
        canvasHeight / 2 -
        this.mainCanvas.viewportTransform[5] * (1 / this.zoom);
      this.minimap.renderAll();
    },
    focusPlayer(player) {
    ...
    resizeCanvas() {
      ...
      this.mainCanvas.setWidth(
        document.querySelector("#invisible").offsetWidth
      );
      this.mainCanvas.renderAll();
    },
