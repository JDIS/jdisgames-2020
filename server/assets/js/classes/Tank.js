...
export class Tank {
  constructor(serverTank) {
    ...
    this.name = new FabricText(
      serverTank.name,
      serverTank.position[0],
      serverTank.position[1] + NAME_OFFSET
    );
    this.healthBar = new HealthBar(serverTank);
    this.score = serverTank.score;
    this.combatLevel = new FabricText(
      "0",
      serverTank.position[0],
      serverTank.position[1]
    );
    this.toCanvas = new fabric.Group([
      this.body,
      this.name.fabricObj,
      this.healthBar.toCanvas,
      this.combatLevel.fabricObj,
    ]);
  ...
  update(newServerTank, hitCallback) {
    ...
    this.position = {
      x: newServerTank.position[0],
      y: newServerTank.position[1],
    };
    this.upgrade_levels = newServerTank.upgrade_levels;
    this.projectile_time_to_live = newServerTank.projectile_time_to_live;
    const combatLevel = Object.values(newServerTank.upgrade_levels).reduce(
      (accumulator, value) => accumulator + value
    );
    this.combatLevel.fabricObj.set("text", combatLevel.toString());

    if (
      newServerTank.current_hp < this.current_hp &&
      newServerTank.max_hp === this.max_hp
    ) {
      hitCallback(this);
    }
  ...
  createDestinationLine() {
    ...
    return new fabric.Line(
      [this.toCanvas.left, this.toCanvas.top, destinationX, destinationY],
      {
        stroke: this.color,
        originX: "left",
        originY: "top",
  ...
  createTargetLine() {
    ...
    return new fabric.Line(
      [this.toCanvas.left, this.toCanvas.top, targetX, targetY],
      {
        stroke: this.color,
